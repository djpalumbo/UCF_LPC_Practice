...
double distanceSqare(point a, point b);
...
//Calculate square of distance between points a and b
double distanceSqare(point a, point b)
{
...
int testT(point a, point b, point m, point c)
{
    ...
    {
        ab = distanceSqare(a, b);
        mc = distanceSqare(m, c);

        //Test if CM = AB
        if(isEqual(ab, mc))
        {
            am = ab/4;
            ac = distanceSqare(a, c);
